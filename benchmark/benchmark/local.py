    ...
    def run(self):
        ...
        for shardId in range(self.shard_num+1):
            for nodeId in range(self.shard_size+1):
                ...
                # data_cmd_MB = f'du -d 1 -h *data_{shardId}_{nodeId}'
                ...
                # node_chaindata_MB = round(float(lines[0].split('\t')[0].replace('M', '').strip()))
                node_statedata_MB = int(lines[1].split('\t')[0])
                # node_statedata_MB = round(float(lines[1].split('\t')[0].replace('M', '').strip()))
